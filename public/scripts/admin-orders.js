function viewOrder(orderId) {
  ...
  xhr.onload = function () {
    if (xhr.status === 200) {
      ...
      document.getElementById("viewDate").value = new Date(order.created_at).toLocaleDateString();
      ...
      if (order.items && order.items.length > 0) {
        order.items.forEach(function (item) {
          itemsHtml +=
            ...
            "<p>Qty: " + item.quantity + " × $" + item.price + " = $" + (item.price * item.quantity).toFixed(2) + "</p>" +
            "</div>";
        });

        itemsHtml += '<div style="margin-top: 15px; font-weight: bold;">Total: $' + order.total + "</div>";
      } else {
